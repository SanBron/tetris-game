...
sc.create_text(535, 780, text="score:", font=("WiGuru 2", 35), fill="black", anchor=NW)
_score = sc.create_text(550, 840, text=str(score), font=("WiGuru 2", 35), fill="black", anchor=NW)
sc.create_text(525, 650, text="record:", font=("WiGuru 2", 35), fill="black", anchor=NW)
_record = sc.create_text(550, 710, text=record, font=("WiGuru 2", 35), fill="black", anchor=NW)
...
while app_running:
    if app_running:
        current_matrix = [[0 for i in range(W)] for j in range(H)]
        record = get_record()
        # move x
        ...
        for row in range(H):
            for col in range(W):
                current_matrix[row][col] = field[row][col]
                if any((figure[i][0] == col and figure[i][1] == row) for i in range(4)):
                    current_matrix[row][col] = 1
        # Display the matrix in the console
        if current_matrix != total_matrix:
            print("Matrix")
            for row in current_matrix:
                print(" ".join(map(lambda x: str(1 if x else 0), row)))
